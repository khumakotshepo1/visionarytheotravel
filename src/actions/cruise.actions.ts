...
export const shipImageAction = async (formData: FormData, id: string) => {
  try {
    const session = await auth();
    ...
    const { rows: name } = await sql.query(
    ...
    // Delete existing images in the folder
    await cloudinary.api.delete_resources_by_prefix(`msc/${name[0].name}`);
    ...
    await sql.query(
      `UPDATE ships SET image = $1 , updated_at = NOW() WHERE ship_id = $2 RETURNING *`,
      [imageUrl, shipId]
    );
...
export const cabinImageAction = async (formData: FormData, id: string) => {
  try {
    const session = await auth();
    ...
    const { rows: cabin } = await sql.query(
      `SELECT * FROM cabins WHERE cabin_id = $1`,
      [cabinId]
    );
    ...
    await cloudinary.api.delete_resources_by_prefix(
      `msc/${cabin[0].ship_id}/${cabin[0].name}`
    );
    ...
    const upload = new Promise(async (resolve, reject) => {
      cloudinary.uploader
        .upload_stream(
          {
            resource_type: "image",
            folder: `msc/${cabin[0].ship_id}/${cabin[0].name}`,
            width: 500, // Set the desired width
            height: 500, // Set the desired height
            crop: "fill", // Crop the image to fit the specified dimensions
          },
    ...
    await sql.query(
      `UPDATE cabins SET image = $1 , updated_at = NOW() WHERE cabin_id = $2 RETURNING *`,
      [imageUrl, cabinId]
    );
