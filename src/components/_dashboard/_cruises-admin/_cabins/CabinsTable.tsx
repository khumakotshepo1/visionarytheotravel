...
export function CabinsTable<TData, TValue>({
  columns,
  data,
  ...
  return (
    <div className="rounded-md text-darkElement dark:text-lightElement">
      <div className="flex items-center py-4">
        <Input
          placeholder="Filter cabins..."
          value={
            (table.getColumn("cabin_name")?.getFilterValue() as string) ?? ""
          }
          onChange={(event) =>
            table.getColumn("cabin_name")?.setFilterValue(event.target.value)
          }
          className="max-w-sm"
        />
      </div>
            ...
            >
              {headerGroup.headers.map((header) => {
                return (
                  <TableHead
                    key={header.id}
                    className="text-darkElement dark:text-lightElement"
                  >
